...
| **swapsize** | `8GB` | 1/2 of System RAM, provides a safety net for OOM events. |
| **maxroot** | `60GB` | Allocate 60GB for `/`. Sufficient for logs, OS, and some ISO storage. |
...
* **Hostname:** `${PVE03_HOSTNAME}`
* **IP Address:** `${PVE03_IP}`
* **Gateway:** `${PVE03_GATEWAY}`
* **DNS:** `${PVE03_DNS}`
